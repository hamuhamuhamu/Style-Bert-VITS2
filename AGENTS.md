...
バージョンは `pyproject.toml` の `version` フィールドで管理。変更時は `style_bert_vits2/constants.py` の `VERSION` も同期させること。

### 依存関係の固定

- `nltk<=3.8.1`: 新しいバージョンでは追加のモデルダウンロードが必要になり、速度低下などの問題が発生するため意図的に固定
- `transformers<4.50.0`: 4.50.0 以降で日本語 BERT がハングアップする問題があるため固定
