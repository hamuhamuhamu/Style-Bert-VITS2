...
Char モデルを固定で使う前提のスクリプトとして `external_spk_emb_gen.py` を追加しています。`train.list` などの list を指定して .spk.npy を生成します。

```
.venv/bin/python external_spk_emb_gen.py --list Data/YourModel/train.list --skip_existing
```
...
```
.venv/bin/python train_ms_nanairo.py -c configs/config_nanairo.json -m Data/YourModel
```
...
.venv/bin/python scripts/speaker_adapter/dump_g.py \
  --config configs/config_nanairo.json \
  --model model_assets/YourModel/G_0.safetensors \
  --list Data/YourModel/train.list \
...
.venv/bin/python scripts/speaker_adapter/apply_axes_demo.py \
  --model model_assets/YourModel/G_0.safetensors \
  --config configs/config_nanairo.json \
  --style_vec model_assets/YourModel/style_vectors.npy \
  --text 'こんにちは' \
